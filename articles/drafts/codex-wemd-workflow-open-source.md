...
我做它的原因也很简单：公众号文章麻烦的地方，往往不在那一段正文，而在素材、图片、标题、摘要、排版、HTML、封面和草稿箱这些重复动作。流程不固定，写一篇就乱一次。
...
```powershell
git clone https://github.com/honestTai/faliang-codex-ex.git
cd faliang-codex-ex
npm install
```
...
所以这套流程不是拿来替你发文的。它只是把重复动作固定住。真正要不要发，还是人来决定。
...
<https://github.com/honestTai/faliang-codex-ex>
